...
const SolarSystem: React.FC<SolarSystemProps> = ({ width = 400, height = 400, speed = 1, showOrbits = true }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const bodiesRef = useRef<Body[]>([]);
  const starsRef = useRef<{ x: number; y: number; alpha: number }[]>([]);

  useEffect(() => {
    bodiesRef.current = PLANETS.map((p) => ({ ...p, angle: Math.random() * Math.PI * 2 }));
  ...
  }, [height, showOrbits, speed, width]);

  return (
    <div className={styles.solarSystem} style={{ width, height }}>
