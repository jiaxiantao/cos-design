...
export function createDandelionField(
  ...
  const initScene = () => {
    const count = clamp(Math.round(options.plantCount ?? 10), 1, MAX_PLANTS);
    const perPlant = Math.max(24, Math.min(90, Math.round(options.seedCount ?? 32)));
    nextPlantId = 1;
  ...
  const startLoop = () => {
    ...
    const removePlant = (plantId: number) => {
      ...
      for (let i = seeds.length - 1; i >= 0; i--) {
        if (seeds[i].plantId === plantId && seeds[i].attached) seeds.splice(i, 1);
      }
