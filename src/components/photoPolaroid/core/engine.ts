  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      restX:
        centerX +
        (t - 0.5) * spreadX * 1.65 +
        (pseudoRandom(index * 7.13 + 2.1) * 2 - 1) * spreadX * 0.42,
      restY:
        centerY -
        Math.sin(t * Math.PI) * spreadY * 0.28 +
        (pseudoRandom(index * 11.37 + 5.9) * 2 - 1) * spreadY * 0.38,
      restRot: (pseudoRandom(index * 3.71 + 1.4) * 2 - 1) * 14 * scatter,
    };
  });
};

export function createPhotoPolaroid(
  container: HTMLElement,
  initial: PhotoPolaroidOptions = { photos: [] },
): PhotoPolaroidController {
  let options: PhotoPolaroidOptions = {
    width: '100%',
    height: 420,
    cardWidth: 150,
    cardHeight: 180,
  ...
  const syncMotionFromRest = () => {
    const rest = buildRestLayouts(
      countOf(),
      viewport.width,
      viewport.height,
      options.cardWidth ?? 150,
      options.cardHeight ?? 180,
      scatterOf(),
    );
    motions.length = 0;
    zOrder.length = 0;
  ...
  const stepPhysics = (time: number) => {
    ...
    for (let i = 0; i < motions.length; i++) {
      ...
      if (
        ...
        Math.abs(m.vRot) > MIN_VELOCITY * 0.35
      ) {
        moving = true;
      } else {
        m.vx = 0;
  ...
  const finishDrag = (event: PointerEvent) => {
    ...
    motion.vx = current.vx * 0.55;
    motion.vy = current.vy * 0.55;
    motion.vRot = current.vx * 0.012;
    ensureLoop();
    paintCard(current.index);
  };
