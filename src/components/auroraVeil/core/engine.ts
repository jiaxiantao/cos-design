...
export function createAuroraVeil(
  ...
  const initScene = () => {
    ...
    const count = Math.max(3, Math.min(8, Math.round(options.bandCount ?? 5)));
    veils.length = 0;
    stars.length = 0;
    pulses.length = 0;
  ...
  const startLoop = () => {
    ...
    const smoothPointer = (dt: number) => {
      ...
      pointer.svx = lerp(pointer.svx, pointer.vx, k * 0.88);
      pointer.vx *= 0.84;
    };

    const flowBias = (time: number) => {
      return pointer.active ? pointer.svx * 0.042 : Math.sin(time * 0.1) * 0.26;
    };
    ...
    const centerX = (veil: Veil, y: number, time: number, sheet: number) => {
      ...
      if (burstLevel > 0.02) {
        x +=
          Math.sin(y * 0.05 + time * 38 + veil.phase) * burstLevel * 12 +
          Math.sin(y * 0.095 - time * 50 + sheet) * burstLevel * 6;
      }
    ...
    const halfWidth = (veil: Veil, y: number, time: number, sheet: number) => {
      ...
      if (burstLevel > 0.02) {
        hw *= 1 + burstLevel * 0.18 * Math.abs(Math.sin(y * 0.085 + time * 44 + sheet));
      }
    ...
    const fillRibbon = (
      ...
      const core = (0.38 + burstLevel * 0.18) * alphaScale * boost * depthFade;
      ...
      vert.addColorStop(0.04, `rgb(${gr} ${gg} ${gb} / ${(0.62 + burstLevel * 0.24) * boost})`);
      vert.addColorStop(0.28, `rgb(${gr} ${gg} ${gb} / ${(0.82 + burstLevel * 0.28) * boost})`);
      vert.addColorStop(0.58, `rgb(${r} ${g} ${b} / ${(0.48 + burstLevel * 0.14) * boost})`);
    ...
    const paintVeils = (time: number) => {
      ...
      for (const veil of veils) {
        for (let sheet = 0; sheet < veil.sheets; sheet++) {
          ...
          fillRibbon(sctx, veil, profile, burstLevel, 1, 0.88 / veil.sheets, time);
        }
      ...
      ctx.globalAlpha = 0.42 + burstLevel * 0.1;
      ctx.filter = 'blur(1px)';
      ctx.drawImage(sheet, 0, 0, width, height);
      ctx.filter = 'none';
      ctx.globalAlpha = 0.62 + burstLevel * 0.12;
