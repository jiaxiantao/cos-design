...
import type { SoapBubblesController, SoapBubblesOptions } from './types';

const P = 'cos-soap-bubbles';
const DEFAULT_W = 800;
const DEFAULT_H = 500;
...
export function createSoapBubbles(
  ...
  const startLoop = () => {
    ...
    const n = clamp(Math.round(options.count ?? 28), 8, 80);
    ...
    const draw = (ts: number) => {
      ...
      const target = clamp(Math.round(options.count ?? 28), 8, 80);
    ...
    draw(0);
    loopCleanup = () => cancelAnimationFrame(frameId);
  };
