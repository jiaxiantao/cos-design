...
const MetaballPool: React.FC<MetaballPoolProps> = ({ width = 400, height = 300, ballCount = 5, color = '#38bdf8' }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const ballsRef = useRef<Ball[]>([]);
  const pointerRef = useRef({ x: -999, y: -999, active: false });

  useEffect(() => {
    const count = Math.max(2, Math.min(ballCount, 10));
  ...
  return (
    <div className={styles.metaballPool} style={{ width, height, '--pool-color': color } as React.CSSProperties}>
