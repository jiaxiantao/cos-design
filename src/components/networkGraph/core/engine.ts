...
export function createNetworkGraph(container: HTMLElement, initial: NetworkGraphOptions = {}): NetworkGraphController {
  ...
  const syncNodes = () => {
    ...
    nodes = nodeList.map((n, i) => {
      ...
      return {
        ...
        label: n.label || n.id,
        color: n.color || '#38bdf8',
        x: width / 2 + Math.cos((i / nodeList.length) * Math.PI * 2) * 140,
  ...
  const handlePointerMove = (e: PointerEvent) => {
    ...
    if (drag) {
      ...
      const nextX = Math.max(r, Math.min(width - r, x + offsetX));
      const nextY = Math.max(r, Math.min(height - r, y + offsetY));
      drag.vx = nextX - drag.lastX;
      drag.vy = nextY - drag.lastY;
      drag.lastX = nextX;
      drag.lastY = nextY;
      node.x = nextX;
      node.y = nextY;
  ...
  const handlePointerUp = (e: PointerEvent) => {
    ...
    if (activeDrag) {
      activeDrag.node.vx = activeDrag.vx * 0.85;
      activeDrag.node.vy = activeDrag.vy * 0.85;
      drag = null;
