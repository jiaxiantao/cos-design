...
const InkBloom: React.FC<InkBloomProps> = ({
  ...
  const propsRef = useRef({ speed, interactive, inkColor });

  useEffect(() => {
    propsRef.current = { speed, interactive, inkColor };
  }, [inkColor, interactive, speed]);
  ...
  useEffect(() => {
    ...
    const onMove = (event: PointerEvent) => {
      if (!propsRef.current.interactive || !event.buttons || !prev) return;
      const p = toSim(event.clientX, event.clientY);
    ...
    const renderStain = () => {
      const [ir, ig, ib] = parseRgb(propsRef.current.inkColor);
  ...
  return (
    <div ref={hostRef} className={styles.inkBloom} style={hostStyle}>
