...
const SmokeFog: React.FC<SmokeFogProps> = ({
  ...
  const spriteColorRef = useRef('');
  const frameRef = useRef(0);
  const configRef = useRef<SmokeConfig>({
    speed,
    disperseStrength,
    disperseRadius,
    interactive,
  ...
  }, [speed, disperseStrength, disperseRadius, interactive, color, backgroundColor]);

  useEffect(() => {
    const tint = parseHex(color, [210, 212, 216]);
    spritesRef.current = createSpritePool(tint);
    spriteColorRef.current = color;
  }, [color]);

  useEffect(() => {
    const count = Math.floor(56 + density * 100);
    puffsRef.current = Array.from({ length: count }, () => {
  ...
  const disperseAt = useCallback(
    (clientX: number, clientY: number) => {
      ...
      const radiusScale = clamp(cfg.disperseRadius, 0, 3);
      const strengthScale = clamp(cfg.disperseStrength, 0, 3);
      // 叠太多会乱，保留最近几次拨风
      if (gustsRef.current.length > 3) gustsRef.current.shift();
  ...
  useEffect(() => {
    ...
    const tint = parseHex(configRef.current.color, [210, 212, 216]);
    if (!spritesRef.current) {
      spritesRef.current = createSpritePool(tint);
      spriteColorRef.current = configRef.current.color;
    }
    ...
    const tick = () => {
      ...
      // 颜色热更新时换贴图池
      if (spriteColorRef.current !== configRef.current.color) {
        const nextTint = parseHex(configRef.current.color, [210, 212, 216]);
        spritesRef.current = createSpritePool(nextTint);
        spriteColorRef.current = configRef.current.color;
      }
      const liveSprites = spritesRef.current ?? sprites;
