...
export function createSmokeFog(
  ...
  const initPuffs = () => {
    const count = Math.floor(56 + (options.density ?? 0.5) * 100);
    puffs.splice(0, puffs.length);
    for (let i = 0; i < count; i++) {
      const progress = Math.pow(Math.random(), 1.75);
      puffs.push(spawnPuff(width, height, progress));
    }
  };
  ...
  const startLoop = () => {
    ...
    const spritePool = sprites;

    let time = 0;
    let localFrameId = 0;

    const paintBackground = () => {
      const [top, mid, bottom] = resolveBackground(options.backgroundColor ?? DEFAULT_BG);
    ...
    const drawPuff = (puff: SmokePuff) => {
      ...
      const sprite = spritePool[puff.sprite];
    ...
    const tick = () => {
      ...
      const motion = clamp(options.speed ?? 1, 0, 3);
      const forceScale = clamp(options.disperseStrength ?? 1, 0, 3);
      time += 0.01 * Math.max(motion, 0.001);
      paintBackground();

      // 颜色热更新时换贴图池
      const nextColor = options.color ?? DEFAULT_COLOR;
      if (spriteColor !== nextColor) {
        const nextTint = parseHex(nextColor, [210, 212, 216]);
        sprites = createSpritePool(nextTint);
        spriteColor = nextColor;
      }
      const liveSprites = sprites ?? spritePool;
