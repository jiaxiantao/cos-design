...
import type { DoublePendulumController, DoublePendulumOptions } from './types';

const P = 'cos-double-pendulum';

export function createDoublePendulum(
  container: HTMLElement,
  initial: DoublePendulumOptions = {},
): DoublePendulumController {
  let options: DoublePendulumOptions = {
    width: 400,
    height: 400,
    trailLength: 120,
    color: '#38bdf8',
  ...
  const start = () => {
    ...
    const trailLength = options.trailLength ?? 120;
    const color = options.color ?? '#38bdf8';
    const color2 = options.color2 ?? '#a78bfa';
    root.className = P;
